...
var map = new ol.Map({
    controls: [
        ...
        new ol.control.OverviewMap({
            collapsed: true,
            layers: [overviewMapBaseLayer, lyr_ward_boundary, lyr_building_ppt, lyr_road_centerline]
        }),
...
var surveyor_submitted_35_gisId = new Set();

function isGisIdSubmitted(gisId) {
    return surveyor_submitted_35_gisId.has(gisId);
}

window.updateSubmittedGisId = function(gisId) {
    surveyor_submitted_35_gisId.add(gisId);
    lyr_building_ppt.changed();
};

window.addEventListener('load', function() {
    try {
        const storedGisIds = JSON.parse(localStorage.getItem('surveyor_submitted_35_gisId') || '[]');
