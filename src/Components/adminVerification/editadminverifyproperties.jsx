...
function EditAdminVerifyProperties() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setOpenSnackbar(true);
        setTimeout(() => {
          navigate('/admin-verification-list');
        }, 2000);
      } else {
  ...
  const handleLogout = () => {
    localStorage.removeItem('admin_verification_token');
...
export default EditAdminVerifyProperties;
