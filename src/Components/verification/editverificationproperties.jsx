...
function EditVerificationProperties() {
    ...
    if (selectedMapWard) {
      ...
      window.addEventListener('message', function(event) {
        if (event.data.type === 'GISID_SELECTED') {
          ...
          setFormData(prev => ({
            ...prev,
            gisId: formattedGisId
          }));
          setGisIdSource('map');
          setSelectedGisInfo(`Ward ${selectedWardOption.displayValue} - ${selectedGISID}`);
          setSuccessMessage(`Selected GIS ID: ${formattedGisId} from Ward ${selectedWardOption.displayValue}`);
  ...
  const openMap = () => {
    const mapUrl = `${window.location.origin}/Gis_finder_ward_01/index.html`;
  ...
  const fetchGisIds = async () => {
    try {
      ...
      if (data.success) {
        ...
        console.log('Updated GIS IDs in localStorage:', {
          surveyor: {
  ...
  useEffect(() => {
    const initializeGisIds = async () => {
      try {
        setLoading(true);
        await fetchGisIds();
        console.log('GIS IDs initialized on component mount');
      } catch (error) {
        console.error('Error initializing GIS IDs:', error);
      } finally {
        setLoading(false);
      }
    };
                ...
                <Box sx={{ display: 'flex', justifyContent: 'center', gap: 2 }}>
                  {!isEditing ? (
                    ...
                    <Button
                      ...
                      color="primary"
                      type="submit"
                      disabled={loading || !hasData || floorData.length === 0 || !review.trim()}
...
export default EditVerificationProperties;
